<x-app-layout>
    <div>
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h2 class="text-3xl font-bold text-gray-900">Tambah Data Gizi</h2>
                <p class="mt-1 text-sm text-gray-500">Input data makanan dan nutrisi pasien</p>
            </div>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route($prefix . '.gizi.store') }}" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="pasien_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            @foreach($pasien as $p)
                                <option value="{{ $p->id }}" {{ old('pasien_id') == $p->id ? 'selected' : '' }}>{{ $p->user->name ?? $p->id }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_makanan" value="{{ old('jenis_makanan') }}" placeholder="Contoh: Nasi, Bubur, Sup" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        @error('jenis_makanan')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah</label>
                        <input type="number" name="jumlah" min="1" value="{{ old('jumlah') }}" placeholder="Jumlah porsi" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="catatan" rows="3" placeholder="Catatan tambahan..." class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('catatan') }}</textarea>
