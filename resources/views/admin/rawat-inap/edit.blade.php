<x-app-layout>
    <div class="max-w-2xl">
        ...
        <div class="mb-6">
            <h2 class="text-3xl font-bold text-gray-900">Edit Rawat Inap</h2>
            <p class="mt-1 text-sm text-gray-500">Perbarui data pasien rawat inap</p>
        </div>

        <div class="bg-white shadow rounded-lg">
            <form action="{{ route($prefix . '.rawat-inap.update', $rawatInap) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <select id="pasien_id" name="pasien_id" required
                        ...
                        @foreach ($pasiens as $pasien)
                            <option value="{{ $pasien->id }}"
                                {{ $rawatInap->pasien_id == $pasien->id || old('pasien_id') == $pasien->id ? 'selected' : '' }}>
                    ...
                    <label for="dokter_id"
                        ...
                        @foreach ($dokters as $dokter)
                            <option value="{{ $dokter->id }}"
                                {{ $rawatInap->dokter_id == $dokter->id || old('dokter_id') == $dokter->id ? 'selected' : '' }}>
                ...
                <div>
                    <label for="tanggal_masuk"
                            ...
                            <input type="date" id="tanggal_masuk" name="tanggal_masuk" required
                                value="{{ old('tanggal_masuk', $rawatInap->tanggal_masuk->format('Y-m-d')) }}"
                ...
                <div>
                    <label for="tanggal_keluar"
                            ...
                            <input type="date" id="tanggal_keluar" name="tanggal_keluar"
                                value="{{ old('tanggal_keluar', $rawatInap->tanggal_keluar ? $rawatInap->tanggal_keluar->format('Y-m-d') : '') }}"
                ...
                <div>
                    <label for="ruang"
                        ...
                        <option value="ICU" {{ $rawatInap->ruang == 'ICU' || old('ruang') == 'ICU' ? 'selected' : '' }}>
                            ICU</option>
                        <option value="Bersalin"
                            {{ $rawatInap->ruang == 'Bersalin' || old('ruang') == 'Bersalin' ? 'selected' : '' }}>
                            Bersalin</option>
                        <option value="Anak"
                            {{ $rawatInap->ruang == 'Anak' || old('ruang') == 'Anak' ? 'selected' : '' }}>Anak
                        </option>
                        <option value="Bedah"
                            {{ $rawatInap->ruang == 'Bedah' || old('ruang') == 'Bedah' ? 'selected' : '' }}>Bedah
                        </option>
                        <option value="Penyakit Dalam"
                            {{ $rawatInap->ruang == 'Penyakit Dalam' || old('ruang') == 'Penyakit Dalam' ? 'selected' : '' }}>
                ...
                <div>
                    <label for="no_tempat_tidur"
                                ...
                                <input type="text" id="no_tempat_tidur" name="no_tempat_tidur" required
                                    value="{{ old('no_tempat_tidur', $rawatInap->no_tempat_tidur) }}"
                ...
                <div>
                    <label for="diagnosa"
                        ...
                        placeholder="Masukkan diagnosa medis">{{ old('diagnosa', $rawatInap->diagnosa) }}</textarea>
                    @error('diagnosa')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="Menunggu"
                            {{ $rawatInap->status == 'Menunggu' || old('status') == 'Menunggu' ? 'selected' : '' }}>
                            Menunggu</option>
                        <option value="Sedang Dirawat"
                            {{ $rawatInap->status == 'Sedang Dirawat' || old('status') == 'Sedang Dirawat' ? 'selected' : '' }}>
                            Sedang Dirawat</option>
                        <option value="Selesai"
                            {{ $rawatInap->status == 'Selesai' || old('status') == 'Selesai' ? 'selected' : '' }}>
                            Selesai</option>
                        <option value="Dirujuk"
                            {{ $rawatInap->status == 'Dirujuk' || old('status') == 'Dirujuk' ? 'selected' : '' }}>
                            Dirujuk</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="catatan" name="catatan" rows="3"
                        ...
                        placeholder="Catatan tambahan (opsional)">{{ old('catatan', $rawatInap->catatan) }}</textarea>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <a href="{{ route($prefix . '.rawat-inap.show', $rawatInap) }}"
