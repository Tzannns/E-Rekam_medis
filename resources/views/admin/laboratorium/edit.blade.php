<x-app-layout>
    <div>
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h2 class="text-3xl font-bold text-gray-900">Edit Hasil Lab</h2>
                <p class="mt-1 text-sm text-gray-500">Perbarui hasil pemeriksaan laboratorium</p>
            </div>
            <div class="space-x-2">
                <a href="{{ route($prefix . '.laboratorium.show', $lab) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Detail</a>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route($prefix . '.laboratorium.update', $lab) }}" class="space-y-4">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="pasien_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            @foreach ($pasienList as $p)
                                <option value="{{ $p->id }}" {{ old('pasien_id', $lab->pasien_id) == $p->id ? 'selected' : '' }}>{{ $p->user->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="dokter_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            @foreach ($dokterList as $d)
                                <option value="{{ $d->id }}" {{ old('dokter_id', $lab->dokter_id) == $d->id ? 'selected' : '' }}>{{ $d->user->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_pemeriksaan" value="{{ old('jenis_pemeriksaan', $lab->jenis_pemeriksaan) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="nilai_rujukan" value="{{ old('nilai_rujukan', $lab->nilai_rujukan) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @error('nilai_rujukan')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Satuan</label>
                        <input type="text" name="satuan" value="{{ old('satuan', $lab->satuan) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="hasil" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('hasil', $lab->hasil) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="tanggal_periksa" value="{{ old('tanggal_periksa', optional($lab->tanggal_periksa)->format('Y-m-d\TH:i')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            <option value="Diajukan" {{ old('status', $lab->status) === 'Diajukan' ? 'selected' : '' }}>Diajukan</option>
                            <option value="Diproses" {{ old('status', $lab->status) === 'Diproses' ? 'selected' : '' }}>Diproses</option>
                            <option value="Selesai" {{ old('status', $lab->status) === 'Selesai' ? 'selected' : '' }}>Selesai</option>
                        </select>
                        @error('status')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Catatan</label>
                        <textarea name="catatan" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('catatan', $lab->catatan) }}</textarea>
