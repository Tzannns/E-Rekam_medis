<x-app-layout>
    <div>
        <div class="mb-6">
            <h2 class="text-3xl font-bold text-gray-900">Edit Data Poli</h2>
            <p class="mt-1 text-sm text-gray-500">Update data poli/poliklinik</p>
        </div>

        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('admin.poli.update', $poli) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                            ...
                            <option value="aktif" @selected(old('status', $poli->status) == 'aktif')>Aktif</option>
                            <option value="tidak_aktif" @selected(old('status', $poli->status) == 'tidak_aktif')>Tidak Aktif</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        class="mt-1 block w-full px-3 py-2 rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $poli->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
