<x-app-layout>
    <div>
        ...
        <form method="POST" action="{{ route('admin.kasir.store') }}" x-data="kasirForm()" class="bg-white shadow rounded p-6">
            @csrf
            ...
            <div class="mt-6">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <button type="button" @click="addItem()" class="px-3 py-1 bg-indigo-600 text-white rounded">Tambah Item</button>
                </div>
                <template x-for="(item, idx) in items" :key="idx">
                    <div class="grid grid-cols-12 gap-2 mb-2">
                        <div class="col-span-5">
                            <input type="text" :name="`items[${idx}][deskripsi]`" x-model="item.deskripsi" placeholder="Deskripsi" class="border rounded w-full px-3 py-2" required>
                        </div>
                        <div class="col-span-2">
                            <input type="number" min="1" :name="`items[${idx}][qty]`" x-model.number="item.qty" class="border rounded w-full px-3 py-2" required>
                        </div>
                        <div class="col-span-3">
                            <input type="number" step="0.01" min="0" :name="`items[${idx}][harga]`" x-model.number="item.harga" class="border rounded w-full px-3 py-2" required>
                        </div>
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="diskon" x-model.number="diskon" class="border rounded w-full px-3 py-2">
                </div>
    ...
    <script>
        function kasirForm() {
            return {
                ...
                diskon: 0,
                addItem() { this.items.push({ deskripsi: '', qty: 1, harga: 0 }); },
                removeItem(i) { this.items.splice(i, 1); },
                subtotal() { return this.items.reduce((s, it) => s + (Number(it.qty) * Number(it.harga)), 0); },
                total() { return Math.max(this.subtotal() - Number(this.diskon || 0), 0); },
                formatCurrency(n) { return Number(n).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
            }
        }
    </script>
</x-app-layout>