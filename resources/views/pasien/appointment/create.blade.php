            ...
            <form method="POST" action="{{ route('pasien.appointment.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="dokter_id" id="dokter_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" {{ (isset($dokterList) && $dokterList->count()) ? '' : 'disabled' }} required>
                            @if(isset($dokterList) && $dokterList->count())
                                <option value="">Pilih Dokter</option>
                                @foreach ($dokterList as $d)
                                    <option value="{{ $d->id }}" {{ (old('dokter_id', $dokterId ?? '') == $d->id) ? 'selected' : '' }}>{{ $d->user->name }} - {{ $d->spesialisasi }}</option>
                                @endforeach
                            @else
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_usulan" id="tanggal_usulan" value="{{ old('tanggal_usulan', $tanggal ?? '') }}" min="{{ date('Y-m-d') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        ...
                        <select name="jadwal_id" id="jadwal_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" {{ (isset($jadwalOptions) && $jadwalOptions->count()) ? '' : 'disabled' }} required>
                            @if(isset($jadwalOptions) && $jadwalOptions->count())
                                ...
                                @foreach ($jadwalOptions as $j)
                                    <option value="{{ $j->id }}" {{ (request('jadwal_id') == $j->id) ? 'selected' : '' }}>{{ $j->jam_mulai }} - {{ $j->jam_selesai }}</option>
                                @endforeach
                            @else
                    ...
                    @if(isset($selectedJadwalId) && $selectedJadwalId && isset($selectedJadwal) && $selectedJadwal)
                        <div class="md:col-span-2">
                            <div class="bg-yellow-50 border border-yellow-200 rounded p-4 mb-3">
                                <p class="text-sm text-yellow-800">Total antrian aktif untuk jadwal ini: <strong>{{ $currentQueueCount }}</strong></p>
                                <p class="text-sm text-yellow-800">Nomor antrian Anda jika mengambil: <strong>{{ $currentQueueCount + 1 }}</strong></p>
                            ...
                            <div class="overflow-x-auto bg-white border rounded">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="3" class="px-6 py-3 text-sm text-gray-500 text-center">Belum ada antrian aktif</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
